...
lib.callback.register(
    ...
    function(source, data)
        ...
        if not encoded or #encoded > 65536 then
            lib.print.error(string.format("%s [%s] tried to generate an oversized outfit code (%d bytes).", GetPlayerName(source), source, encoded and #encoded or 0))
            return false, "Your outfit is too large to share."
        end
